...
- Tool wrappers map host exceptions into stable execbox error categories.
...
- No ambient Node globals injected by execbox itself
...
Execbox keeps a fixed error vocabulary so callers and MCP wrappers can reason about failures consistently.
...
| `tool_error`          | Host tool implementation failed in a non-execbox-specific way   |
