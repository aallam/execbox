 ...
 *   7. Memory footprint delta during sustained execution
 *
 * Usage:
 *   node --import tsx scripts/benchmark-full.ts [--iterations=N] [--warmups=N] [--suite=name]
 *
...
type DisposableExecutor = Executor & {
...
const pooledConfig = {
  pool: { maxSize: 2, minSize: 0, idleTimeoutMs: 30_000, prewarm: true },
};

const factories: ExecutorFactory[] = [
  { name: "quickjs (in-process)", create: () => new QuickJsExecutor() },
  {
  ...
  {
    name: "worker (pooled)",
    create: () => new WorkerExecutor(pooledConfig) as DisposableExecutor,
  },
  {
  ...
  {
    name: "process (pooled)",
    create: () => new ProcessExecutor(pooledConfig) as DisposableExecutor,
  },
];
...
async function suiteColdStart(): Promise<void> {
  ...
  for (const f of factories) {
    ...
    // Cold: create fresh executor each time, no prewarm, measure first execution
    ...
    // Warm: create executor, prewarm, then measure first execution
    for (let i = 0; i < coldRuns; i++) {
      const executor = f.create();
      try {
        await executor.prewarm?.(1);
    ...
    console.log(`\n  [${f.name}]`);
    printStats("cold (no prewarm)", coldSamples);
...
async function suiteMemory(): Promise<void> {
  console.log("\n" + "=".repeat(80));
  console.log("SUITE: Memory Footprint During Sustained Execution");
  console.log("=".repeat(80));
